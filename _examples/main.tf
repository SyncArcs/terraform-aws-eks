...
module "ssh" {
  source      = "git::https://github.com/SyncArcs/terraform-aws-security-group?ref=v1.0.0"
  name        = "${local.name}-ssh"
  environment = local.environment
  vpc_id      = module.vpc.id
...
module "kms" {
  source              = "git::https://github.com/SyncArcs/terraform-aws-kms?ref=v1.0.0"
  name                = "${local.name}-kms2"
  environment         = local.environment
  enabled             = true
...
module "eks" {
  ...
  map_additional_iam_users = [
    {
      userarn  = "arn:aws:iam::329599634843:user/SyncArcs"
      username = "test"
